...
const STATUS_LABELS: Record<string, string> = {
  generating: "生成中", checking: "质检中", rewriting: "改写中", saving: "保存中",
};
...
function GlobalProgressBar() {
  ...
  return (
    <div style={{
      padding: `${space[2]} ${space[3]}`, background: colors.primaryMuted,
      borderRadius: "6px", marginBottom: space[3], border: `1px solid ${colors.primaryBorder}`,
    }}>
      ...
      {active.tokensReceived > 0 && (
        <div style={{ ...font.xs, color: colors.textDim }}>{active.tokensReceived} tokens</div>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: colors.bg, color: colors.fg, ...scrollbar }}>
      ...
      <div className="mobile-header" style={{
        ...
        alignItems: "center", padding: `0 ${space[4]}`, gap: space[3], zIndex: 90,
      }}>
        <button onClick={() => setSidebarOpen(true)} style={{
          display: "flex", alignItems: "center", justifyContent: "center",
          width: "36px", height: "36px", border: "none", background: "transparent",
          cursor: "pointer", color: colors.fg, borderRadius: radius.sm,
        }}>
      ...
      <div className="sidebar-overlay" onClick={() => setSidebarOpen(false)} style={{
        display: sidebarOpen ? "block" : "none", position: "fixed", inset: 0,
      ...
      <aside className={`app-sidebar${sidebarOpen ? " open" : ""}`} style={{
        ...
        <div style={{ fontFamily: fonts.display, fontSize: "20px", fontWeight: 700, padding: `0 24px 24px`, color: colors.fg, letterSpacing: ".02em" }}>
          AI <span style={{ color: colors.accent }}>小说家</span>
        </div>
          ...
          {NAV_ITEMS.map((item, index) => {
            ...
            return (
              <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)} style={{
                ...
                borderRadius: "6px", fontSize: "14px", color: isActive ? colors.accent : colors.fgSecondary,
                background: isActive ? colors.accentBg : "transparent", fontWeight: isActive ? 500 : 400,
        ...
        <div style={{ padding: `0 10px` }}>
          <button onClick={toggleTheme} style={{
            ...
            color: colors.fgSecondary, background: "transparent", border: "none",
