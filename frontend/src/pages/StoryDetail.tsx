...
const severityColors: Record<string, string> = {
  critical: colors.danger, high: "#f97316", medium: "#eab308", low: colors.textMuted,
};

const severityBg: Record<string, string> = {
  critical: colors.dangerBg, high: "rgba(249,115,22,0.12)", medium: "rgba(234,179,8,0.12)", low: "rgba(113,113,122,0.1)",
};
...
export default function StoryDetail() {
  ...
  const handleCancel = () => { if (id) cancelGeneration(id); };
  ...
  const writeNextBlocking = async () => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "写作失败"); }
    finally { setBlockingWrite(false); }
  };
  ...
  const writeAll = async () => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "批量写作失败"); }
    finally {
    ...
    <div>
      ...
      {error && (
        <div style={{ padding: `${space[2]} ${space[4]}`, marginBottom: space[3], borderRadius: "6px", fontSize: "13px",
          background: error === "重写完成" ? "rgba(34,197,94,0.12)" : colors.dangerBg,
          color: error === "重写完成" ? colors.primary : colors.danger }}>
          {error}
        </div>
      )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: space[3] }}>
            ...
            <span style={{ ...font.sm, fontWeight: 600, color: colors.primary }}>
              第{gen.chapterNumber}章 &middot; {
                ...
                gen.status === "checking" ? "质检中..." : gen.status === "rewriting" ? "改写中..." :
                gen.status === "saving" ? "保存中..." : gen.status
              }
            </span>
            {gen.tokensReceived > 0 && <span style={{ ...font.xs, color: colors.textMuted }}>({gen.tokensReceived} tokens)</span>}
            <div style={{ width: "80px", height: "4px", background: colors.bgElevated, borderRadius: "2px", overflow: "hidden", marginLeft: space[2] }}>
              <div style={{ height: "100%", width: "30%", background: colors.primary, borderRadius: "2px", animation: "progressGlow 1.5s ease-in-out infinite" }} />
            </div>
          </div>
          <button onClick={handleCancel} style={{ ...btn.ghost, padding: `${space[1]} ${space[3]}`, fontSize: "12px" }}>取消</button>
        </div>
      )}

      {gen && gen.status === "error" && (
        <div style={{ padding: `${space[2]} ${space[4]}`, marginBottom: space[3], borderRadius: "6px", fontSize: "13px", background: colors.dangerBg, color: colors.danger }}>
      ...
      {writingAll && (
        <div style={{ ...card, marginBottom: space[4], padding: `${space[3]} ${space[4]}`, borderColor: colors.warningBorder, background: colors.warningBg }}>
          ...
          <div style={{ height: "4px", background: "rgba(245,158,11,0.15)", borderRadius: "2px" }}>
            <div style={{ height: "100%", width: `${writeAllProgress.target > 0 ? (writeAllProgress.current / writeAllProgress.target) * 100 : 0}%`, background: colors.warning, borderRadius: "2px", transition: "width 0.5s" }} />
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: space[3], marginBottom: space[5], flexWrap: "wrap" }}>
        ...
        <button onClick={writeNextBlocking} disabled={streaming || writingAll || blockingWrite} style={{ ...btn.ghost, opacity: streaming || writingAll || blockingWrite ? 0.6 : 1 }}>
      ...
      <div style={{ display: "flex", gap: "4px", marginBottom: space[5], borderBottom: `1px solid ${colors.border}`, opacity: focusMode ? 0 : 1, transition: "opacity 0.2s" }}>
        ...
        <button onClick={() => setFocusMode(!focusMode)} style={{
          padding: `${space[1]} ${space[3]}`, fontSize: "12px", background: "none",
          border: `1px solid ${focusMode ? colors.primaryBorder : colors.border}`, borderRadius: "4px",
          cursor: "pointer", color: focusMode ? colors.primary : colors.textMuted,
          display: "flex", alignItems: "center", gap: space[1], marginBottom: space[2],
        }}>
          {focusMode ? "退出专注" : "专注模式"}
        </button>
      </div>
        ...
        <div style={{ display: "flex", gap: space[5], opacity: focusMode ? 0.3 : 1, transition: "opacity 0.2s" }}>
          <div style={{ width: focusMode ? "0" : "220px", flexShrink: 0, overflow: focusMode ? "hidden" : "auto", transition: "width 0.3s ease" }}>
            {chapters.length === 0 ? (
              ...
              chapters.map((ch) => (
                <button key={ch.chapter_number} onClick={() => loadChapter(ch.chapter_number)} style={{
                  display: "block", width: "100%", textAlign: "left", padding: `${space[2]} ${space[2]}`,
                  fontSize: "12px", background: selectedChapter?.chapter_number === ch.chapter_number ? "rgba(34,197,94,0.1)" : "transparent",
                  border: "none", borderRadius: "4px", color: selectedChapter?.chapter_number === ch.chapter_number ? colors.primary : colors.fgSecondary,
                  cursor: "pointer", marginBottom: "2px", transition: "background 0.15s",
                }}>
          ...
          <div style={{ flex: 1, minWidth: 0, maxWidth: focusMode ? "800px" : "100%", margin: focusMode ? "0 auto" : "0", transition: "max-width 0.3s ease, margin 0.3s ease" }}>
            {selectedChapter ? (
              <ChapterContent chapter={selectedChapter} rewriting={rewriting} focusMode={focusMode} onRewrite={rewriteChapter} />
            ) : (
              ...
              return (
                <div key={fs.id} style={{ ...card, padding: `${space[3]} ${space[4]}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: space[3], marginBottom: space[1] }}>
                    <span style={{ padding: "2px 8px", fontSize: "11px", borderRadius: "4px", fontWeight: 600, background: statusStyle.bg, color: statusStyle.color, border: `1px solid ${statusStyle.border}` }}>
...
function ChapterContent({ chapter, rewriting, focusMode, onRewrite }: {
  chapter: ChapterDetail; rewriting: boolean; focusMode: boolean; onRewrite: () => void;
}) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: space[4] }}>
        <h3 style={{ fontSize: "15px", color: colors.primary, margin: 0 }}>
          第{chapter.chapter_number}章 &middot; {chapter.title}
          <span style={{ marginLeft: space[3], fontSize: "12px", color: colors.textMuted, fontWeight: 400 }}>
            {chapter.content ? chapter.content.replace(/\s/g, "").length : 0}字
          </span>
        </h3>
        <button onClick={onRewrite} disabled={rewriting} style={{ ...btn.ghost, padding: `${space[1]} ${space[3]}`, fontSize: "12px", opacity: rewriting ? 0.6 : 1 }}>
          {rewriting ? "重写中..." : "重写本章"}
        </button>
      </div>

      {(chapter.check_results?.length > 0 || chapter.rewrites_count > 0) && (
        <div style={{ display: "flex", gap: space[1], marginBottom: space[4], flexWrap: "wrap" }}>
          {chapter.check_results?.map((cr, i) => (
            <span key={i} style={{ padding: "2px 6px", fontSize: "10px", borderRadius: "3px",
              background: cr.passed ? colors.primaryMuted : colors.dangerBg,
              color: cr.passed ? colors.primary : colors.danger,
              border: `1px solid ${cr.passed ? colors.primaryBorder : colors.dangerBorder}`, fontWeight: 500 }}>
              {cr.layer} {cr.passed ? "\u2713" : "\u2717"}
              {cr.issues_count > 0 && ` (${cr.issues_count})`}
            </span>
          ))}
          {chapter.rewrites_count > 0 && (
            <span style={{ padding: "2px 6px", fontSize: "10px", borderRadius: "3px", background: colors.warningBg, color: colors.warning, border: `1px solid ${colors.warningBorder}`, fontWeight: 500 }}>
              已改写{chapter.rewrites_count}次
            </span>
          )}
        </div>
      )}

      {chapter.check_results?.some((cr) => cr.issues_count > 0) && <QualityCheckDetails checkResults={chapter.check_results} />}

      <div style={{ ...card, padding: focusMode ? space[8] : space[6], fontSize: focusMode ? "16px" : "15px", lineHeight: focusMode ? "2.0" : "1.9", whiteSpace: "pre-wrap" as const, color: colors.fg, maxHeight: focusMode ? "85vh" : "70vh", overflow: "auto", position: "relative", boxShadow: focusMode ? "inset 0 0 100px rgba(0,0,0,0.3)" : "none", transition: "all 0.3s ease" }}>
        {chapter.content || "暂无内容"}
      </div>
      {focusMode && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, pointerEvents: "none", background: "radial-gradient(ellipse at center, transparent 50%, rgba(0,0,0,0.4) 100%)", zIndex: -1 }} />
      )}
    </div>
  );
}

function QualityCheckDetails({ checkResults }: { checkResults: any[] }) {
  return (
    <div style={{ ...card, marginBottom: space[4], padding: space[4] }}>
      ...
      {checkResults.filter((cr) => cr.issues_count > 0).map((cr) => (
        <div key={cr.layer} style={{ marginBottom: space[3] }}>
          ...
          {cr.issues?.map((issue: any, j: number) => (
            <div key={j} style={{ padding: `${space[2]} ${space[3]}`, marginBottom: space[1], borderRadius: "4px", background: severityBg[issue.severity] || "transparent", border: `1px solid ${severityColors[issue.severity]}20` }}>
              <div style={{ display: "flex", gap: space[2], alignItems: "center" }}>
                <span style={{ padding: "1px 5px", fontSize: "10px", borderRadius: "3px", background: severityColors[issue.severity], color: "#fff", fontWeight: 600 }}>{issue.severity}</span>
                <span style={{ ...font.xs, color: colors.text }}>{issue.type}: {issue.description}</span>
              </div>
              {issue.suggestion && (
                <div style={{ ...font.xs, color: colors.textMuted, marginTop: space[1], paddingLeft: space[6] }}>建议: {issue.suggestion}</div>
              )}
            </div>
          ))}
        </div>
